...
public:
    ArmNode() : Node("arm_node") {
        ...
        std::string dh_path = this->get_parameter("dh_file").as_string();
        joints_ = readDH(dh_path);

        // Parametro per la frequenza di aggiornamento (Hz)
        this->declare_parameter<double>("publish_rate", 10.0);
        double rate = this->get_parameter("publish_rate").as_double();

        // Genera traiettoria (esempio: cerchio)
        //trajectory_points_ = generateHeartTrajectory( 0.05, Eigen::Vector3d(0, 0.0, 0), 50);
...
private:
    void update() {
        ...
        Eigen::Vector3d tcp = forward_kinematics(joints_);
        ...
        // Aggiorna marker della traiettoria
        ...
        // Passa al prossimo punto (ciclo continuo)
